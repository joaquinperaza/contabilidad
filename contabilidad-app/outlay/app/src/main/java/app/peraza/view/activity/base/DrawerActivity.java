...
import app.peraza.Constants;
import app.peraza.R;
import app.peraza.domain.model.User;
import app.peraza.view.activity.SingleFragmentActivity;
import app.peraza.view.alert.Alert;
import app.peraza.view.fragment.AboutFragment;
...
public abstract class DrawerActivity extends ParentActivity {
    ...
    public void setupDrawer(User currentUser) {
        ...
        if (currentUser.isAnonymous()) {
            items.add(new PrimaryDrawerItem().withName(app.peraza.R.string.menu_item_create_user).withIcon(MaterialDesignIconic.Icon.gmi_account_add).withIdentifier(ITEM_CREATE_USER));
        }
        Activity that =this;
        ...
        if (!o.toString().equals(currentUser.getId())) {
            ...
            if (o.toString().hashCode() > 1 || o.toString().hashCode() < -1) {
                ...
                i2++;
                    ...
                    sat = true;
                    items.add(new PrimaryDrawerItem().withName(app.peraza.R.string.menu_item_analysis).withIcon(MaterialDesignIconic.Icon.gmi_trending_up).withIdentifier(ITEM_ANALYSIS));
                    items.add(new PrimaryDrawerItem().withName(app.peraza.R.string.menu_item_categories).withIcon(MaterialDesignIconic.Icon.gmi_apps).withIdentifier(ITEM_CATEGORIES));
                    items.add(new PrimaryDrawerItem().withName(app.peraza.R.string.menu_item_feedback).withIcon(MaterialDesignIconic.Icon.gmi_email).withIdentifier(ITEM_FEEDBACK));
                    ...
                    items.add(new PrimaryDrawerItem().withName(app.peraza.R.string.menu_item_signout).withIcon(MaterialDesignIconic.Icon.gmi_sign_in).withIdentifier(ITEM_SING_OUT));
                                ...
                                if (iDrawerItem != null) {
                                    ...
                                    switch (id) {
                                        ...
                                        case ITEM_FEEDBACK:
                                            ...
                                            try {
                                                startActivity(Intent.createChooser(emailIntent, getString(app.peraza.R.string.label_send_email)));
                                            } catch (android.content.ActivityNotFoundException ex) {
                                                Alert.error(getRootView(), getString(app.peraza.R.string.error_no_email_clients));
                                            }
                                            break;
                                        case ITEM_ABOUT:

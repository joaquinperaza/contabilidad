...
import app.peraza.utils.DeviceUtils;
import app.peraza.utils.IconUtils;
import app.peraza.view.helper.AnimationUtils;
import app.peraza.view.helper.TextWatcherAdapter;
import app.peraza.view.helper.ViewHelper;
...
public class LoginForm extends RelativeLayout {
    ...
    @Bind(app.peraza.R.id.signInForm)
    View signInForm;

    @Bind(app.peraza.R.id.signUpForm)
    View signUpForm;

    @Bind(app.peraza.R.id.fab)
    FloatingActionButton fab;

    @Bind(app.peraza.R.id.signUpButton)
    Button signUpButton;

    @Bind(app.peraza.R.id.signInButton)
    Button signInButton;

    @Bind(app.peraza.R.id.forgetPassword)
    Button forgetPassword;

    @Bind(app.peraza.R.id.skipButton)
    Button skipButton;

    @Bind(app.peraza.R.id.signInInputEmail)
    TextInputLayout signInEmail;

    @Bind(app.peraza.R.id.signInInputPassword)
    TextInputLayout signInPassword;

    @Bind(app.peraza.R.id.signUpInputEmail)
    TextInputLayout signUpEmail;

    @Bind(app.peraza.R.id.signUpInputPassword)
    TextInputLayout signUpPassword;

    @Bind(app.peraza.R.id.signUpInputRepeatPassword)
    ...
    private void init(AttributeSet attrs) {
        ...
        View parent = inflater.inflate(app.peraza.R.layout.view_login_form, this, true);
    ...
    private boolean isSignInInputValid() {
        ...
        if (!isEmailValid(signInEmail.getEditText().getText())) {
            ...
            signInEmail.setError(getContext().getString(app.peraza.R.string.error_signin_invalid_email));
            result = false;
        }
        String password = signInPassword.getEditText().getText().toString();
        if (TextUtils.isEmpty(password) || password.length() < 6) {
            signInPassword.setErrorEnabled(true);
            signInPassword.setError(getContext().getString(app.peraza.R.string.error_signin_invalid_password));
            result = false;
        }
        return result;
    }
    ...
    private boolean isSignUpInputValid() {
        ...
        if (!isEmailValid(signUpEmail.getEditText().getText())) {
            ...
            signUpEmail.setError(getContext().getString(app.peraza.R.string.error_signin_invalid_email));
            result = false;
        }
        ...
        if (TextUtils.isEmpty(password) || password.length() < 6) {
            ...
            signUpPassword.setError(getContext().getString(app.peraza.R.string.error_signin_invalid_password));
            result = false;
            return result;
        }
        ...
        if (!password.equals(repeatPassword)) {
            ...
            signUpRepeatPassword.setError(getContext().getString(app.peraza.R.string.error_signin_password_match));
            result = false;
        }
